...
local function run(msg, matches)
  if matches[1]:lower() == 'lionbot' then -- Put everything you like :)
    local about = _config.about_text
    local name = user_print_name(msg.from)
    savelog(msg.to.id, name.." ["..msg.from.id.."] used /lionbot ")
    return about
  end 
  ...
  if matches[1]:lower() == "stats" then
    ...
    if matches[2] == "lionbot" then -- Put everything you like :)
...
return {
  patterns = {
    ...
    "^([Ss]tatslist)$",
    "^([Ss]tats) (group) (%d+)",
    "^([Ss]tats) (lionbot)",-- Put everything you like :)
		"^[!/]([Ll]ionbot)"-- Put everything you like :)
    }, 
  run = run
}
